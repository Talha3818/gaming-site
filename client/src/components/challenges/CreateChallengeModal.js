...
const CreateChallengeModal = ({ onClose, onSubmit, userBalance }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate minimum 30 minutes in the future
    const now = new Date();
    const minTime = new Date(now.getTime() + 30 * 60 * 1000);
    
    if (scheduledDateTime <= minTime) {
      alert('Match must be scheduled at least 30 minutes in the future');
      return;
    }
        ...
        <div className="p-6 space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-xs text-dark-300 mb-1">Match Duration</label>
                <select
                  value={matchDuration}
